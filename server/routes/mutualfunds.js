...
router.get('/', (req, res) => {
  const rows = db.prepare('SELECT * FROM mutual_funds WHERE user_id = ? ORDER BY created_at DESC').all(req.session.userId);
  res.json(rows.map(withMfCalcs));
});

router.get('/:id', (req, res) => {
  const row = db.prepare('SELECT * FROM mutual_funds WHERE id = ? AND user_id = ?').get(req.params.id, req.session.userId);
...
router.post('/', (req, res) => {
  ...
  const stmt = db.prepare(`
    INSERT INTO mutual_funds
      (user_id, fund_name, amc, category, folio_number, purchase_nav, current_nav, units, investment_mode, investment_date, broker, notes)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `);
  const info = stmt.run(
    req.session.userId, fund_name, amc || null, category || null, folio_number || null, purchase_nav,
    current_nav || 0, units, investment_mode || 'Lump Sum', investment_date || null, broker || null, notes || null
  );
  const row = db.prepare('SELECT * FROM mutual_funds WHERE id = ? AND user_id = ?').get(info.lastInsertRowid, req.session.userId);
  res.status(201).json(withMfCalcs(row));
});

router.put('/:id', (req, res) => {
  const existing = db.prepare('SELECT * FROM mutual_funds WHERE id = ? AND user_id = ?').get(req.params.id, req.session.userId);
  ...
  `).run(
    ...
    merged.broker, merged.notes, req.params.id, req.session.userId
  );
  const row = db.prepare('SELECT * FROM mutual_funds WHERE id = ? AND user_id = ?').get(req.params.id, req.session.userId);
  res.json(withMfCalcs(row));
});
...
router.delete('/:id', (req, res) => {
  const info = db.prepare('DELETE FROM mutual_funds WHERE id = ? AND user_id = ?').run(req.params.id, req.session.userId);
